...
OLSR::recv_olsr(Packet* p) {
	...
	for (int i = 0; i < op->count; i++) {
		...
		//printf("\nmyID[%d]\trecv[%d]\n",ra_addr(),ih->saddr());//j15203
		//printf("Source[%d]\tRecieve[%d]\n",ih->saddr() , ra_addr());//j1520

                //RSCH
		if (hellocount_.find(ra_addr()) != hellocount_.end()) { //自分がマップにあるかどうか(外)

			//送信者がマップにあるかどうか(中)
    			if (hellocount_[ra_addr()].find(ih->saddr()) != hellocount_[ra_addr()].end()) {
        			//有る場合
        			hellocount_[ra_addr()][ih->saddr()]++;
    			} else {
        			//無かった場合
        			hellocount_[ra_addr()][ih->saddr()] = 1;
    			}

		} else {
    			//中身が無かった場合
    			std::map<nsaddr_t, int>::map new_m;
   			new_m[ih->saddr()] = 1;
   			hellocount_[ra_addr()] = new_m;
		}
		
		//printf("[%f]TIME\n",CURRENT_TIME);
	//	printf("| sender[%d]->receiver[%d] : HELLO[%d]", 
        //		ih->saddr(), ra_addr(), hellocount_[ra_addr()][ih->saddr()]);
                //RSCH
	
	//printf("\n-------------------------------------\n\n");
	//printf("\n^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n\n");

	if (recvcount_.find(ih->saddr()) != recvcount_.end()) { //自分がマップにあるかどうか(外)
                        //送信者がマップにあるかどうか(中)                     
                        if (recvcount_[ih->saddr()].find(ra_addr()) != recvcount_[ra_addr()].end()) {
                                //有る場合
                                recvcount_[ih->saddr()][ra_addr()]++;
                        } else {
                                //無かった場合
                                recvcount_[ih->saddr()][ra_addr()] = 1; 
                        }
                        ...
                        std::map<nsaddr_t, int>::map new_m;
                        new_m[ra_addr()] = 1; 
                        recvcount_[ih->saddr()] = new_m;
                }
		//std::cout << "[" << CURRENT_TIME << "]" << std::endl;
		printf("[%f] SENDER[%d]->RECEIVER[%d] : COUNT[%d]\n", 
        		CURRENT_TIME,ih->saddr(), ra_addr(), recvcount_[ih->saddr()][ra_addr()]);

/*
	std::map<std::pair<nsaddr_t, nsaddr_t>,int>recvcount_;
	std::map<std::pair<nsaddr_t, nsaddr_t>,int>::iterator r;
	r = recvcount_.find(make_pair(ih->saddr(), ra_addr()));
	if (r != recvcount_.end()) {
		r->second()++;
	} else {
		recvcount_.insert(std::make_pair(make_pair(ih->saddr(), ra_addr(), 1)));
	}
	std::cout << "SENDER= " << ih->saddr() << "RECEIVER= " << ra_addr() << "COUNT= " << r->second() << std::endl;
*/	
//added 2018/7/16～
/*	std::vector<recvcount*> vec1;
	std::vector<recvcount*> vec2;

	recvcount* recv1 = new recvcount;
	recvcount* recv2 = new recvcount;
	
	static int r = 0;
	if (r == 0){	
	recv1->sender_ = ra_addr(); recv1->receiver_ = ih->saddr(); recv1->count_ = 0;
	//recv1->sender_ = ih->saddr(); recv1->receiver_ = ra_addr(); recv1->count_ = 0;
	//recv2->sender_ = 0; recv2->receiver_ = 0; recv2->count_ = 0;
	
	vec1.push_back(recv1);
	//vec1.push_back(recv2);
	r = 1;
	std::cout << "初期化されました" << std::endl;
	}
	//std::cout << "r = " << r << std::endl;
	for (std::vector<recvcount*>::iterator it = vec1.begin(); it!= vec1.end(); it++) {
		recvcount* rc = *it;
		if (rc->sender() == ih->saddr()) {
			if (rc->receiver() == ra_addr()) {
				rc->count()++;
				std::cout << "countインクリメント" << std::endl;
				break;
			}else{
				recv1->sender_ = ih->saddr();
				recv1->receiver_ = ra_addr();
				recv1->count_ = 1;
				vec2.push_back(recv1);
				std::cout << "受信者追加" << std::endl;
				break;
			}
		}else{
			recv2->sender_ = ih->saddr();
			recv2->receiver_ = ra_addr();
			recv2->count_ = 1;
			vec2.push_back(recv2);
			std::cout << "送信者追加" << std::endl;
			break;
		}
	}
	for (std::vector<recvcount*>::iterator it = vec2.begin(); it != vec2.end(); it++) {
		recvcount* rc = *it;
		vec1.push_back(rc);
	}
	vec2.clear();
	...
	for (std::vector<recvcount*>::iterator it = vec1.begin(); it != vec1.end(); it++) {
		recvcount* rc = *it;
		std::cout << "送信= " << rc->sender() << " 受信= " << rc->receiver() << " カウント= " << rc->count() << "\n" << std::endl;
	}
*/	//ここまで
	}
...
#ifdef PRINT
			printf("my[%d]local[%d]\tnb[%d]tasyn[%f]\tlost[%f]\ttime[%f]\n",
				OLSR::node_id(ra_addr()),
				OLSR::node_id(link_tuple->local_iface_addr()),
				OLSR::node_id(link_tuple->nb_iface_addr()),
					link_tuple->sym_time(),
            				link_tuple->asym_time(),
            				link_tuple->lost_time(),
            				link_tuple->time());
#endif
				...
				if (link_tuple->nb_iface_addr() == nb_tuple->nb_main_addr())
					nb_main_addr = true;
				//added 2018/7/09
					std::cout << "node" << ra_addr() << "-->" << "node" << nb_tuple->nb_main_addr() << std::endl;
					std::cout << "dist = 1\n"<< std::endl;
				}
			}
	...
	for (nb2hopset_t::iterator it = nb2hopset().begin(); it != nb2hopset().end(); it++) {
		...
		if (ok) {
			...
			rtable_.add_entry(nb2hop_tuple->nb2hop_addr(),
					...
					2);
			//added 2018/7/09
			std::cout << "node" << ra_addr() << "-->" << "node" << nb2hop_tuple->nb2hop_addr() << std::endl;
			std::cout << "dist = 2\n"<< std::endl;
		}
	}
	...
	for (u_int32_t h = 2; ; h++) {
		...
		for (topologyset_t::iterator it = topologyset().begin();
			...
			if (entry1 == NULL && entry2 != NULL && entry2->dist() == h) {
				...
				added = true;
				//added 2018/7/09
				std::cout << "node" << ra_addr() << "-->" << "node" << topology_tuple->dest_addr() << std::endl;
				std::cout << "dist = " << entry2->dist()+1 << "\n" << std::endl; 
			}
		}
...
/*	for (mprset_t::iterator it = mprset().begin(); it != mprset().end(); it++) {
	...
	printf("\n^^^^^^^^^^^^^^^Recieve^^^^^^^^^^^^^^^\n");
	printf("[%f]TIME\n",CURRENT_TIME);
	std::cout << "| Receivernode = 【" << receiver_iface << "】"  << std::endl;
	printf("| recv_hello[%d]\n",hcount); //j15203ns
	//printf("\n------------------------------------\n\n");*/
}
...
OLSR::enque_msg(OLSR_msg& msg, double delay) {
	...
	//static int enques_hello = 0;
	//static int enques_tc = 0;
	//std::cout << "JITTER = "<< JITTER-delay <<std::endl;		
	msgs_.push_back(msg);
	OLSR_MsgTimer* timer = new OLSR_MsgTimer(this);
	timer->resched(delay);
//added 2018/06/29	
	//std::cout << "msgs_size= " << sizeof(msgs_) << std::endl;
	//std::cout << "msgsize= " << sizeof(msg) << std::endl;
...
//added 2018/06/25
	...
	enques_hello++;
		printf("\n---------------HELLO-----------------\n");
		printf("[%f]TIME\n",CURRENT_TIME);
		std::cout << "| Sendernode = 【" << ra_addr() << "】 "  << std::endl; //added 2018/7/02
		printf("| delayhello[%f]\n",delay); //j15203
		//printf("| enques_hello[%d]\n",enques_hello);
		printf("-------------------------------------\n\n");
	}
...
OLSR::send_pkt() {
	...
	//added 2018/06/29
	//std::cout << "msgs_size= " << msgs_.size() << std::endl;
